...
from scripts.common.yearbook_flow_common import parse_station_meta, sample_id_from_name
...
def resolve_reciprocal_matches(
    ...
    for csv_entry in csv_entries:
        ...
        image_margin = best.score - (
            image_ranked[1].score if len(image_ranked) > 1 else 0.0
        )
        is_confirmed = (
    ...
    if len(average_tokens) >= 2:
        if year_stats:
            anchor = next(
                (
                    token
                    for token in average_tokens
                    if abs(token.center_y - year_stats[0].center_y) > 24.0
                ),
                average_tokens[0],
            )
        else:
